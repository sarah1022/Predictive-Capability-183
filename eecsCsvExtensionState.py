...
def parseCSVFile(fileName):
    ...
    # For each detail line of the current state,
    # the key (state name) and the desired entries
    # are added to the list 'data'. The key 'name'
    # and 'data' are appended to the dictionary
    # hurState as a key-value pair.
        data = []
        count = 0
        while count < numEntries:
            ...
            detLine[1] = float(detLine[1])
            if detLine[2] != '0':
                detLine[2] = float(detLine[2])
            if detLine[3] != '0':
                detLine[3] = float(detLine[3])
                detLine[4] = float(detLine[4])
                detLine[5] = float(detLine[5])
                detLine[6] = float(detLine[6])
                detLine[7] = float(detLine[7])
                detLine[8] = float(detLine[8])
                detLine[9] = float(detLine[9])
                detLine[10] = float(detLine[10])
...
def retrieveEntireAttributeState(dictionary, key, columnNo):
    keylist = dictionary.keys()
    keylist.sort()
    list1 = []
    if key in keylist:
...
def retrieveKeyListState(dictionary, key):
    keylist = dictionary.keys()
    if key in keylist:
        return True
    else:
